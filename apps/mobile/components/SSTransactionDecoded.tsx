...
const t = tn('transaction.decoded')
    ...
    <>
      <TouchableOpacity onPress={toggleDisplay}>
        <SSHStack gap="sm" style={{ justifyContent: 'flex-end' }}>
          <SSText color="muted">
            {display === 'list' ? t('btnCollapse') : t('btnExpand')}
          </SSText>
          {display === 'list' ? (
...
function SSTransactionDecodedItem({
  ...
  return (
    <SSVStack gap="none">
      <SSText weight="bold">{t(`label.${field}`, { ...placeholders })}</SSText>
      <SSText color="muted">{value}</SSText>
      <SSText color="muted" size="xxs">
        {t(`description.${field}`, { ...placeholders })}
      </SSText>
    </SSVStack>
  )
}
