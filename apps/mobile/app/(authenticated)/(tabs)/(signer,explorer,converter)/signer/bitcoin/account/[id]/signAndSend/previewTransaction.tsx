...
function mapBuildTransactionError(error: unknown): string {
  const errorMessage = error instanceof Error ? error.message : String(error);
  const lower = errorMessage.toLowerCase();
  if (lower.includes("dust")) {
    return t("transaction.error.previewBuildFailedDust");
  }
  return errorMessage;
}
    ...
    async function getTransaction() {
      ...
      if (outputs.length === 0) {
        if (!cancelled) {
          setPsbtBuildStatus("error");
          setPsbtBuildErrorMessage(
            t("transaction.error.previewMissingOutputs")
          );
        }
        return;
      }
      ...
      try {
        ...
        const errorMessage = mapBuildTransactionError(error);
        setPsbtBuildStatus("error");
        setPsbtBuildErrorMessage(errorMessage);
  ...
  const combineAndFinalizeMultisigPSBTs = () => {
    try {
      ...
      const allInputsReady =
        combinedPsbt.data.inputs.every(hasEnoughSignatures);
