...
const VECTOR_4_CHILD_PAIR_HEX = VECTOR_4_HEX.replace(
  '07d90130a1018401f480f4',
  '07d90130a101838400f401f480f4'
)
...
const OUTPUT_DESCRIPTOR_V3_PK_HEX =
...
describe('crypto-output descriptors', () => {
  ...
  it('substitutes @0 placeholders in BCR-2023-010 output-descriptor v3', () => {
    const descriptor = decodeCryptoOutputCbor(
      Buffer.from(OUTPUT_DESCRIPTOR_V3_PK_HEX, 'hex')
    )
    expect(descriptor).toBe(
  ...
  it('decodes a child-pair component as <0;1>/*', () => {
    const descriptor = decodeCryptoOutputCbor(
      Buffer.from(VECTOR_4_CHILD_PAIR_HEX, 'hex')
    )
    expect(descriptor).toMatch(
      /^pkh\(\[d34db33f\/44'\/0'\/0'\]xpub[a-zA-Z0-9]+\/<0;1>\/\*\)$/
    )
  })

  it('defaults omitted children to <0;1>/* for account-level HD keys', () => {
    const withoutChildren = VECTOR_4_HEX.replace(
      'a5035821',
      'a4035821'
    ).replace('07d90130a1018401f480f4', '')
    const descriptor = decodeCryptoOutputCbor(
      Buffer.from(withoutChildren, 'hex')
    )
    expect(descriptor).toMatch(
      /^pkh\(\[d34db33f\/44'\/0'\/0'\]xpub[a-zA-Z0-9]+\/<0;1>\/\*\)$/
    )
