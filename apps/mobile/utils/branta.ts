...
import { BRANTA_ID_PARAM, BRANTA_SECRET_PARAM } from '@/constants/branta'
...
function hasBrantaZkQueryParams(content: string): boolean {
  ...
  return params.has(BRANTA_ID_PARAM) && params.has(BRANTA_SECRET_PARAM)
}
...
async function openBrantaVerifyUrl(url: string | undefined): Promise<void> {
  if (!url) {
    return
  }
