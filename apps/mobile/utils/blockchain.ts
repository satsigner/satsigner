...
export class MemPoolOracle implements BlockchainOracle {
    ...
    async get(endpoint: string) {
        return fetch(this.baseUrl + endpoint).then((response: any) => response.json())
    }
    async getText(endpoint: string) {
        return fetch(this.baseUrl + endpoint).then((response: any) => response.text())
    }
    ...
    async getCurrentBlockHeight(): Promise<number> {
        return this.getText(`/blocks/tip/height`)
            .then((height: string) => Number(height))
    }
    ...
    async getCurrentDifficulty(): Promise<number> {
        const data: any = await this.get(`/api/v1/mining/hashrate/1d`)
    ...
    async getDifficultyAdjustment(): Promise<DifficultyAdjustment> {
        const data : any = await this.get(`/blocks/tip/hash`)
    ...
    async getTransaction(txid: string): Promise<Tx> {
        const data : any = await this.get(`/api/tx/${txid}`)
        return data as Tx
    }
    async getTransactionHex(txid: string): Promise<string> {
        const data : string = await this.getText(`/api/tx/${txid}`)
        return data
    }
    async getTransactionOutspends(txid: string): Promise<TxOutspends> {
        const data : any = await this.getText(`/api/tx/${txid}/outspends`)
        return data as TxOutspends
    }
    async getTransactionStatus(txid: string): Promise<TxStatus> {
        const data : any = await this.getText(`/api/tx/${txid}/status`)
