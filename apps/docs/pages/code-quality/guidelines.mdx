...
Page components are `TSX` functions located under `@/app/src`. They may:
...
Do not parse error or do casting:

```typescript
try {
  // code
} catch (error) {
   const errorMessage = error instanceof Error ? error.message : t('ecash.error.nfcEmissionFailed')
...
In a try/catch, if the error is not handled because the function does not apply
to some scenarios, then the try/catch should be removed and the code should not
run for the particular scenario.
